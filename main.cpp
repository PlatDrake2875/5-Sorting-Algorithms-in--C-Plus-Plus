...
ifstream in("t.in");
ofstream out("t.out");
//ofstream out("proiect_print_data.xlsx");
int N, A_Max;
...
int get_bucket(int x, int i)
{
    return ((x >> (i * BUCKET_SIZE)) & RADIX_SIZE);
}

void Counting_Sort_Radix(vector<int> &A, vector<int> &B, int bucket)
{
...
*/
void Shell_Sort(vector<int> &A, int gap_type)
{
    vector<int> Gaps;
    if(gap_type == 1)
    {
...
void heapify(vector<int> &A, int node, int N)
{
    int Nod_Max = node;
    int l = 2 * node;
    int r = 2 * node + 1;
...
void check_if_sorted(vector<int> A)
{
    for(int i = 1; i < N; ++i)
    {
        if(A[i] > A[i + 1])
...
void sortari(int cnt, vector<int> A)
{
    //auto t = clock();
...
int main()
{
    ...
    while(T--)
    {
        ...
        for(int i = 1; i <= N; ++i)
        {
            nums[i] = uniform_int_distribution<int>(-A_Max + 1, A_Max)(rng);
        }
